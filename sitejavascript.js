...
function maximizeRestore() {
  ...
  if (isMaximized) {
    ...
    windowElement.style.transitionProperty =
      "width, height, top, left, border-radius";
    ...
    autoResize = () => {
      // Store reference to the callback function
      windowElement.style.width = `${window.innerWidth}px`;
      windowElement.style.height = `${window.innerHeight}px`;
    };
...
function handleTouchMove(event) {
  ...
  let newX = initialWindowX + touch.clientX - initialMouseX;
  let newY = initialWindowY + touch.clientY - initialMouseY;
  ...
  // Clamp the position within the window
  newX = Math.max(windowLeft, Math.min(newX, windowRight));
  newY = Math.max(windowTop, Math.min(newY, windowBottom));
