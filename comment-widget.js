...
// admin login
const s_adminUsername = 'tesia';  // Your username
let v_adminHash = null;           // Will fetch from your sheet
let v_isAdmin = false;            // Admin login status
...
const v_formHtml = `
    ...
    <div id="c_nameWrapper" class="c-inputWrapper">
        ...
        <input class="c-input c-nameInput" name="entry.${s_nameId}" id="entry.${s_nameId}" type="text" maxlength="${s_maxLengthName}" placeholder="Enter your name..." required>
    </div>
    ...
    <input class="c-input c-websiteInput" name="entry.${s_websiteId}" id="entry.${s_websiteId}" type="text" placeholder="Optional — link, handle, or email" maxlength="100">
</div>
...
if (s_commentsOpen) {
    ...
    adminBtn.style.cssText = 'margin-top:1rem; padding:0.4rem 0.8rem; background:#1e3a8a; color:white; border:none; border-radius:4px; cursor:pointer; font-size:0.85rem; display:block; width:100%;';
    adminBtn.onclick = showAdminLogin;
    c_form.appendChild(adminBtn);
}
...
function showAdminLogin() {
    ...
    modal.id = 'c_adminModal';
    modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:9999;display:flex;justify-content:center;align-items:center;font-family:inherit;';
    modal.innerHTML = `
        <div style="background:#fff;padding:2rem;border-radius:8px;border:2px solid #000;max-width:300px;text-align:center;">
            <h3 style="margin:0 0 1rem;">🔑 Admin Login</h3>
            <input id="c_adminUser" type="text" placeholder="Username" style="width:100%;padding:0.5rem;margin-bottom:0.5rem;border:1px solid #000;border-radius:4px;box-sizing:border-box;">
            <input id="c_adminPass" type="password" placeholder="Password" style="width:100%;padding:0.5rem;margin-bottom:1rem;border:1px solid #000;border-radius:4px;box-sizing:border-box;">
            <div style="display:flex;gap:0.5rem;justify-content:center;">
                <button onclick="checkAdminLogin()" style="padding:0.5rem 1rem;background:#000;color:#fff;border:none;border-radius:4px;cursor:pointer;">Login</button>
                <button onclick="document.getElementById(\'c_adminModal\').remove()" style="padding:0.5rem 1rem;background:#ccc;border:none;border-radius:4px;cursor:pointer;">Cancel</button>
            </div>
        </div>
    `;
    document.body.appendChild(modal);
    document.getElementById('c_adminPass').focus();
}

// pwd check
function checkAdminLogin() {
    const user = document.getElementById('c_adminUser').value;
    const pass = document.getElementById('c_adminPass').value;
    const hash = btoa(user + ':' + pass);
    
    if (hash === v_adminHash) {
        v_isAdmin = true;
        document.getElementById('c_adminModal').remove();
        // Auto-fill your name as admin
        document.getElementById(`entry.${s_nameId}`).value = 'Tesia (Admin)';
        alert('✅ Logged in as Tesia!');
    } else {
        alert('❌ Wrong username or password');
        document.getElementById('c_adminPass').value = '';
    }
}
...
async function getComments() {
    ...
    try {
        ...
        if (adminJson.table.rows?.[0]?.c?.[0]?.v) {
            v_adminHash = adminJson.table.rows[0].c[0].v;
        }
...
function createComment(data) {
    ...
    if (filteredName.includes('Tesia (Admin)') || filteredName === 'tesia' || filteredName === s_adminUsername) {
        name.classList.add('owner');
        name.title = 'Verified site owner';
    }
